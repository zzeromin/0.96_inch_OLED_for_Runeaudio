...
def main():
    ...
    x = padding
    # Load default font.
    font_alb = ImageFont.truetype('/root/miniOLED_for_Runeaudio/NanumGothic.ttf', 13)
    poller = MPDPoller()
    poller.connect()
    
    font_tit = ImageFont.truetype('/root/miniOLED_for_Runeaudio/NanumGothic.ttf', 13)
    font_tm = ImageFont.truetype('/root/miniOLED_for_Runeaudio/NanumGothic.ttf', 14)
    while True:
        draw.rectangle((0,0,width,height), outline=0, fill=0)
        status = poller.poll()
        if status is None:
